...
export const Route = createFileRoute("/admin/_layout/dashboard")({ component: AdminDashboard });
    ...
    <div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={stats.data?.customRequestCount ?? "—"}
        />
      </div>
      <div className="mt-10 rounded-sm border border-border bg-card">
        ...
        {recent.length === 0 ? (
          ...
          <ul className="divide-y divide-border">
            {recent.map((order) => (
              <li key={order.id} className="flex items-center justify-between px-6 py-4 text-sm">
                <div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-ink">{formatUSD(Number(order.total))}</span>
                </div>
              </li>
            ))}
          </ul>
