...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      { title: "Your Cart — BT Collection LLC" },
...
function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <Header />
      <PageHeader kicker="Your Bag" title="Shopping" italic="Cart" blurb="Review your selection before continuing to checkout." />
        ...
        {items.length === 0 ? (
          <div className="grid place-items-center py-20 text-center">
            ...
            <p className="mt-3 max-w-sm text-sm text-foreground/70">
              Explore our collection of faith-inspired pieces made to be given, gifted, and treasured.
            </p>
            <Link
              to="/shop"
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1.6fr_1fr]">
              ...
              {items.map((it) => (
                <li key={`${it.id}-${it.variant ?? ""}`} className="flex gap-4 py-6 md:gap-6">
                  <Link to="/product/$slug" params={{ slug: it.id }} className="shrink-0">
                    <img src={it.img} alt="" className="h-28 w-24 rounded-sm object-cover md:h-32 md:w-28" />
                  </Link>
                  <div className="flex min-w-0 flex-1 flex-col">
                    ...
                    <div className="mt-auto flex items-center justify-between pt-4">
                      <div className="inline-flex items-center rounded-full border border-border">
                        <button aria-label="Decrease" onClick={() => setQty(it.id, it.qty - 1, it.variant)} className="grid h-9 w-9 place-items-center text-foreground/70 hover:text-ink"><Minus className="h-3 w-3" /></button>
                        <span className="w-6 text-center text-sm">{it.qty}</span>
                        <button aria-label="Increase" onClick={() => setQty(it.id, it.qty + 1, it.variant)} className="grid h-9 w-9 place-items-center text-foreground/70 hover:text-ink"><Plus className="h-3 w-3" /></button>
                      </div>
                      <button
            ...
            <aside className="h-fit rounded-sm border border-border bg-cream/50 p-6 md:p-8">
              ...
              <dl className="mt-6 space-y-3 border-b border-border pb-6 text-sm">
                ...
                <div className="flex justify-between"><dt className="text-foreground/75">Shipping</dt><dd className="text-ink">{shipping === 0 ? "Free" : formatUSD(shipping)}</dd></div>
                <div className="flex justify-between text-[11px] uppercase tracking-[0.2em] text-muted-foreground"><dt>Taxes</dt><dd>Calculated at checkout</dd></div>
              </dl>
